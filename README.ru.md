...
adgui передаёт CLI минимальное desktop/XDG-окружение (пользователь, локаль, display/session) и запускает `adguardvpn-cli` без controlling terminal, чтобы `sudo` не спрашивал пароль в терминале, из которого стартовал adgui. Wrapper сначала использует уже закэшированные credentials sudo (`sudo -n`). Если их нет — askpass. Диалог пароля в GUI появляется только когда нет валидного ticket; пароль сохраняется в runtime-файл `.pass` (mode `0600`) для askpass и затирается при выходе.

В `~/.config/adgui/adguirc` можно задать:

- `ADGUARD_CMD` — путь к `adguardvpn-cli`
- `ADGUARD_KILL_CMD` — неинтерактивная команда завершения (например `/usr/bin/sudo -n kill -TERM`)
- `ADGUARD_SUDO_WRAP=0` — отключить wrapper (для passwordless sudo / отладки)
