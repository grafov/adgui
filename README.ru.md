...
Нативное десктопное GUI (Go + Fyne) для AdGuard VPN на Linux — Wayland или XLibre/X11.
Не веб-страница в браузере: обычное оконное приложение, которое управляет `adguardvpn-cli`.
...
- **Нативное приложение для Linux** — работает на Wayland или XLibre/X11 как обычное окно, а не JS в браузере
- **Фичи adguardvpn-cli** — управление исключениями сайтов (общий/выборочный режим), подключение по списку локаций, действия в трее (connect/disconnect)
- **Проверка региона по IP** — как GeoIP-сервисы определяют ваш egress IP; этой проверки нет в CLI
- **Работа с sudo** — поддержка привилегированного `sudo`, которого требует `adguardvpn-cli` в TUN-режиме (диалог askpass или passwordless `sudo -n`)
...
- **Вставить**: читает буфер обмена по нажатию (или Alt+Ctrl+V на вкладке «Домены»). На Wayland дашборд может кратко замереть: чтение буфера (`readDataOfferAsString`) блокирует поток UI — известное ограничение Fyne/GLFW, не фоновый опрос
